...
// version is the build's version string, handed in by Main from the root package where the
// -X main.version ldflag stamps it. It stays in package main because that is where the release
// build injects it; this is a copy for the code that reports it.
var version string

// templates is the built-in specs FS, handed in by Main from the root embed. See templates.go
// for why the embed itself cannot live here.
var templates fs.FS
...
// Main is the program, moved out of package main so the repository root holds only the thin
// entry point and the embed. ver and examples come from there - ver because the release ldflag
// stamps main.version, examples because //go:embed cannot reach examples/ from a subdirectory -
// and argv is os.Args. It returns the process exit code rather than calling os.Exit, so the one
...
func Main(ver string, examples embed.FS, argv []string) int {
	...
	if argv[1] == "serve" {
		record(argv[1], argv[1:], nil)
	}

	err := dispatch(argv[1], argv[2:])
