...
| **Deploy it anywhere** | `sbx pack` writes the image for a platform that gives one container and one HTTP port; `sbx connect` turns that back into ordinary local ports, so `psql` connects to a database running somewhere else |
