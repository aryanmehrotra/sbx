...
| **Watch a deployed sandbox** | `sbx ui --connect https://sbx.example.dev` - the same dashboard, reading a sandbox that is somewhere else. Several deployments merge into one screen, and it is read-only on purpose: a connect token carries bytes and reports what is fronted, so it cannot wake, cap or destroy anything |
