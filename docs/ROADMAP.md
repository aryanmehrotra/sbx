...
`pvc` volumes and disk-snapshot forks included (v0.12), and its warm pool (v0.13). Not yet: forking a VM's memory under
another name (a snapshot fork is its disk); `files`, `init` and host volumes into a VM; the
OpenSandbox API through a helper VM (refused at startup); the
